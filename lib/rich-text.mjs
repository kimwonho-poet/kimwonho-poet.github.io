...
export function renderRich(doc, namespace = '') {
  validateRich(doc);
  const prefix = /^[a-zA-Z0-9_-]*$/.test(namespace) ? namespace : '';
  const notes = [];
  const render = node => {
    if (node.type === 'text') {
    ...
    if (node.type === 'footnote') {
      ...
      return `<sup class="footnote-ref"><a id="${prefix}note-ref-${number}" href="#${prefix}note-${number}" data-note-target="${prefix}note-${number}" aria-label="각주 ${number}">${number}</a></sup>`;
    }
