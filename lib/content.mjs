...
import { validateDetail } from './activities.mjs';
    ...
    value.groups.forEach(g => {
      ...
      g.items.forEach(item => {
        ...
        if (item.id !== undefined) {
          string(item.id, 100);
          if (!/^[a-zA-Z0-9_-]+$/.test(item.id) || ids.has(item.id)) fail('상세 페이지 주소가 중복되거나 올바르지 않습니다.');
          ids.add(item.id);
        }
        if (item.detail !== undefined) validateDetail(item.detail);
      });
    });
  } else {
